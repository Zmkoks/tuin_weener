...
export default function Home({ planten, plekkenVanaf, beplanting }: { planten: Plant[]; plekkenVanaf: Plek[]; beplanting: Record<string,string[]> }){
 ...
 {tab==='plattegrond'&&<section className="workspace map-workspace"><p className="eyebrow">INTERACTIEVE PLATTEGROND</p><h1>Waar groeit wat?</h1><p className="lead">Klik op een plantvak, een boom of een heester om te zien wat daar groeit. Dubbelklik op een plek om direct naar <em>Wat groeit hier?</em> te gaan.</p><div className="map-layout"><div className="large-map"><Plattegrond zones={plekken} gekozen={zoneId} onKies={setZoneId} onDubbelklik={(id)=>window.location.assign(`/plek/${encodeURIComponent(id)}`)} namen={zoneNamen} plants={plants} placements={placements}/></div><aside><div className="zone-result">{zone?<><h2>Wat groeit hier?</h2><p>{zonePlants.length} {zonePlants.length===1?'plant':'planten'} op deze plek</p>{zonePlants.map(p=><Link className="zone-plant" href={`/plant/${p.slug}`} key={p.slug}><PlantFoto plant={p}/><span><b>{p.naam}</b></span><strong aria-hidden="true">→</strong></Link>)}</>:<><h2>Kies een plek</h2><p>Klik op de kaart op een vak, boom of heester om te zien wat daar groeit.</p></>}</div></aside></div></section>}

 {tab==='pdfs'&&<section className="workspace"><p className="eyebrow">DRUKWERK</p><h1>Altijd een actuele versie.</h1><p className="lead">Download documenten opnieuw wanneer een exemplaar beschadigd is of informatie is gewijzigd.</p><div className="pdf-list"><a href="/drukwerk/boekje"><span>PDF</span><div><b>Compleet plantenboekje</b><small>38 pagina’s · alle planten</small></div><strong>Printen / PDF →</strong></a><a href="/drukwerk/plattegrond"><span>PDF</span><div><b>Plattegrond met plantenindex</b><small>A4 staand · actuele versie printen</small></div><strong>Printen / PDF →</strong></a><a href="/pdf/handleiding.pdf" download><span>PDF</span><div><b>Handleiding plantenbordjes</b><small>A4, klaar om opnieuw te printen</small></div><strong>Download ↓</strong></a></div><div className="notice"><b>Nieuwe informatie toegevoegd?</b><p>Na het opslaan kan hier automatisch een nieuwe drukversie worden klaargezet.</p><button>Nieuwe PDF’s maken</button></div></section>}
 </main>
}
